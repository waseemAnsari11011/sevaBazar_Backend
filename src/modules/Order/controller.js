...
exports.createOrder = async (req, res) => {
  ...
  try {
    ...
    for (const vendor of vendors) {
      for (const productInfo of vendor.products) {
        ...
        for (const orderedVariation of productInfo.variations) {
          ...
          if (productVariation) {
            ...
            if (productVariation.quantity < 0) {
              return res.status(400).json({
                error: `Insufficient quantity for variation ${orderedVariation._id}`,
              });
            }
          } else {
            return res.status(400).json({
              error: `Variation with ID ${orderedVariation._id} not found in product ${productInfo.product}`,
            });
          }
        }
        ...
        const totalQuantity = product.variations.reduce(
          (sum, variation) =>
            variation.parentVariation === null ? sum + variation.quantity : sum,
          0
        );
    ...
    // ---- ASYNC SECTION ----
    // Now handle the delivery notifications asynchronously
    processDeliveryNotifications(savedOrder);
  } catch (error) {
    await session.abortTransaction();
    session.endSession();
    res.status(400).json({ error: error.message });
...
const processDeliveryNotifications = async () => {
  try {
    const deliveryPersonnel = await Delivery.find({ isRestricted: false });
    ...
    const pushNotificationPromises = deliveryPersonnel.map((deliveryPerson) => {
      ...
      if (deliveryPerson.fcmDeviceToken) {
        return sendPushNotification(
          ...
          "New Order Available",
          `A new order has been placed. Please check the details.`
        );
      } else {
        // Resolve immediately if there is no token
    ...
    // Log a success message after all notifications are sent
    console.log(
      "All push notifications have been successfully sent to delivery personnel."
    );
  } catch (error) {
...
exports.getNewOrdersCountByVendor = async (req, res) => {
  try {
    ...
    console.error(error);
    res.status(500).json({
      message: "An error occurred while retrieving new orders count.",
    });
  }
};
...
exports.markOrderViewed = async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "Vendor-specific orders marked as viewed" });
  } catch (error) {
    res.status(500).json({
      message: "An error occurred while marking orders as viewed",
      error: error.message,
    });
  }
};
